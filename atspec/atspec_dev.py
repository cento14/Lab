...
class pointsource:
 def __init__(self,evtfile="",insfile="",l=0.,b=-100., name=''):
  ...
  if evtfile == '' :
    evtfile = input('Enter events file : ')
    
  if insfile == '' :
    insfile = input('Enter response file : ')
    
  if b == -100 :
      l =  input('Enter G. longitude : ')
      b =  input('Enter G. latitude : ')
 ...
 def getspec(self, emin=1., emax=100., nch=10, outfile='', sigmas=3., verbose=False):
    ...
    self.canali_min=  numpy.array(list(range(int(nch)))) *self.sb  +lemin
  ...
  #print aeff
      ...
      if verbose :
        print(numpy.round(10.**(lene_sig+self.sb/2.),3), ' ', int(n_on),' ', numpy.round(n_off*alpha,2),' ', numpy.round(sigm,2) ,' ', sigm > sigmas)
